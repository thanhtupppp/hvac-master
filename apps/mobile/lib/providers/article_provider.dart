...
final articlesByCategoryProvider = StreamProvider.family<List<Article>, String>(
  (ref, category) {
...
class Brand {
  ...
  factory Brand.fromFirestore(DocumentSnapshot doc) {
    ...
    return Brand(id: doc.id, name: data['name'] as String? ?? doc.id);
  }
}
...
final articlesByCategoryAndBrandProvider =
    StreamProvider.family<List<Article>, Map<String, String>>((ref, params) {
...
final allArticlesProvider = StreamProvider<List<Article>>((ref) {
  final firestore = ref.watch(firestoreProvider);
  return firestore.collection('articles').snapshots().map((snapshot) {
