...
class TemperatureConverter {
  ...
  static double fromCelsius(double celsius, TemperatureUnit to) {
    switch (to) {
      ...
      case TemperatureUnit.fahrenheit: return (celsius * 9 / 5) + 32;
...
class TemperatureDeltaConverter {
  static double toKelvin(double value, TemperatureDeltaUnit from) {
    switch (from) {
      case TemperatureDeltaUnit.celsius:    return value;
      case TemperatureDeltaUnit.fahrenheit:  return value * 5 / 9;
  ...
  static double fromKelvin(double kelvin, TemperatureDeltaUnit to) {
    switch (to) {
      case TemperatureDeltaUnit.celsius:   return kelvin;
      case TemperatureDeltaUnit.fahrenheit: return kelvin * 9 / 5;
      case TemperatureDeltaUnit.kelvin:    return kelvin;
    }
  }

  static double convert(double value, TemperatureDeltaUnit from, TemperatureDeltaUnit to) {
    if (from == to) return value;
  ...
  static String label(TemperatureDeltaUnit unit) {
    switch (unit) {
      case TemperatureDeltaUnit.celsius:    return '°C';
      case TemperatureDeltaUnit.fahrenheit:  return '°F';
