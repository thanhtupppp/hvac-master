...
class WarningBuilder {
  static List<HvacWarning> buildWarnings({
    ...
    if (isOutOfRange) {
      warnings.add(HvacWarning(
        type: WarningType.frictionOutOfRange,
        message: 'Tỷ lệ ma sát nằm ngoài phạm vi khuyến nghị (${unitSystem == UnitSystem.imperial ? '0.05–0.30 in.wg/100ft' : '0.4–2.5 Pa/m'}).',
        severity: WarningSeverity.info,
      ));
    }
  ...
  static HvacWarning? checkAspectRatio(double aspectRatio) {
    if (aspectRatio > 4.0) {
      return HvacWarning(
        ...
        message: 'Tỷ lệ cạnh (${aspectRatio.toStringAsFixed(1)}) vượt quá giới hạn khuyến nghị (4:1).',
        severity: WarningSeverity.warning,
      );
    }
    return null;
