      ...
      children: [
        for (final u in [
          ...
          if (u != from)
            _buildResultRow(
              PowerConverter.label(u),
              PowerConverter.convert(_inputValue, from, u).toStringAsFixed(3),
              accent,
            ),
      ],
    );
    ...
    return ListView(
      ...
      children: [
        for (final u in PressureConverter.all)
          if (u != from)
            _buildResultRow(
              '${PressureConverter.label(u)} (${PressureConverter.description(u)})',
    ...
    return ListView(
      ...
      children: [
        for (final u in TemperatureUnit.values)
          if (u != from)
            _buildResultRow(
              '${TemperatureConverter.label(u)} (${_tempLabel(u)})',
